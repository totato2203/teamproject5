...
<div class="container">
   <form id="frm01" action="${path }/freeBoardInsert.do"
...
function insertProc(){
	if(confirm("등록하시겠습니까?")){
		var freeTitleVal = $("[name=freeTitle]").val();
		if(freeTitleVal == ""){
			alert("제목을 입력하세요");
			$("[name=freeTitle]").focus();
			return; // 프로세스를 중단 처리
		}
		var freeWriterVal = $("[name=freeWriter]").val();
		if(freeWriterVal == ""){
			alert("작성자를 입력하세요");
			$("[name=freeWriter]").focus();
			return; // 프로세스를 중단 처리
		}
