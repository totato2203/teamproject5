...
<div class="contentbox">
	...
	<input id="nextbtn1" class="w-btn w-btn-indigo " type="button" value="다음문제" onclick="goNext()" /> 
	<input id="nextbtn2" class="w-btn w-btn-indigo " type="button" value="힌트" onclick="#" />	<!--location.href를 통해서 이동할 수 있도록 한다.  -->
	</form>
	...
	%>
	<p>대중들에게도 탄소중립이란 단어가 익숙해질정도로 빈번하게 들려오고 있습니다.	<!-- 데이터베이스에서 문제를 끌고 온다.  -->
	 그만큼 기상이변이 날마다 심각해지고 있다는 것을 반증하는 것이며 국가와 더불어 모든 사람들이 지구가 되돌릴 수 없는 상황에 이르지 않게 노력해야합니다.
	 이 페이지는 탄소중립이 어째서 중요한지, 국가적으로 이루어지고 있는 탄소중립 전략은 무엇인지, 가정속의 탄소중립을 실천하기 위해서는 무엇을 해야하는지에 대해서 함께 알아보기 위해 제작되었습니다. 함께 알아볼까요?
	</p>
	...
	function goNext(){
		...
		alert("정답입니다!");
		document.cookie = "fift_ans=1";
		}else{
			alert("오답입니다!");
		document.cookie = "fift_ans=0";
		}
		var point=Number(tot_point())
		console.log("숫자로 넘어왔니?"+"%d",point)
		
		if(point>3){
		location.href="ResultSuccess.jsp";
		}else{
			location.href="ResultFail.jsp";	
		}
	}
	function tot_point(){
		var tot=0;
		if(fir_ans=getCookie('fir_ans'))
			tot++;
		if(sec_ans=getCookie('sec_ans'))
			tot++;
		if(thir_ans=getCookie('thir_ans'))
			tot++;
		if(four_ans=getCookie('four_ans'))
			tot++;
		if(fift_ans=getCookie('fift_ans'))
			tot++;		
		return tot;
	}
